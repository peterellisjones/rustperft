...
pub fn king_moves<L: List>(board: &Board, to_mask: BB, list: &mut L) {
    let stm = board.stm();
    ...
    let targets = king_moves_from_sq(from) & to_mask & not_friendly;
    let pushes = targets & board.bb_empty();
    let captures = targets ^ pushes;
    list.add_pushes(from, pushes);
    list.add_captures(from, captures);
}

pub fn knight_moves<L: List>(board: &Board, to_mask: BB, from_mask: BB, list: &mut L) {
    let stm = board.stm();
    ...
    for (from, _) in movers.iter() {
        let targets = knight_moves_from_sq(from) & to_mask & not_friendly;
        let pushes = targets & board.bb_empty();
        let captures = targets ^ pushes;
        list.add_pushes(from, pushes);
        list.add_captures(from, captures);
...
const KING_MOVES: [BB; 64] = [BB(0x0000000000000302u64),
...
const KNIGHT_MOVES: [BB; 64] = [BB(0x0000000000020400u64),
