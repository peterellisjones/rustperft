...
const DOUBLE_PAWN_PUSH_FLAG: u8 = 64;
...
impl Move {
    ...
    pub fn is_double_pawn_push(&self) -> bool {
        ((self.upper & DOUBLE_PAWN_PUSH_FLAG) != 0) && ((self.lower & (!63)) == 0)
    }
    ...
    pub fn is_ep_capture(&self) -> bool {
        ((self.lower & (!63)) == CAPTURE_FLAG) && ((self.upper & (!63)) == CAPTURE_FLAG)
    }
    ...
    pub fn new_ep_capture(from: Square, to: Square) -> Move {
        Move {
            lower: from.to_u8() | CAPTURE_FLAG,
            upper: to.to_u8() | DOUBLE_PAWN_PUSH_FLAG,
        }
    }

    pub fn new_double_pawn_push(from: Square, to: Square) -> Move {
        Move {
            lower: from.to_u8(),
            upper: to.to_u8() | DOUBLE_PAWN_PUSH_FLAG,
        }
...
mod test {
    ...
    #[test]
    fn double_pawn_push() {
        let mv = Move::new_double_pawn_push(B2, B4);
        ...
        assert_eq!(mv.is_double_pawn_push(), true);
    }
    ...
    fn push() {
        ...
        assert_eq!(mv.is_promotion(), false);
        assert_eq!(mv.is_ep_capture(), false);
        assert_eq!(mv.is_double_pawn_push(), false);
    }
